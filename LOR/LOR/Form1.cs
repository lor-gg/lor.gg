...
namespace LOR
{
    ...
    public partial class Form1 : Form
    {
        ...
        async private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    myDeck = JsonConvert.DeserializeObject<Deck>(data);
                    DrawDeck();
        ...
        // This updates, but it's really shit because it doesn't account for previous updates, so if you update twice without playing any cards, it will act like you have 2 copies of every card in hand
        async private void update_Click(object sender, EventArgs e)
        {
            // In the class
            HttpClient client = new HttpClient();
            ...
            try
            {
                HttpResponseMessage response = client.GetAsync("/positional-rectangles").Result;  // Blocking call! Program will wait here until a response is received or a timeout occurs.
                if (response.IsSuccessStatusCode)
                {
                    // Parse the response body.
                    var data = await response.Content.ReadAsStringAsync();
                    Board boardState = JsonConvert.DeserializeObject<Board>(data);

                    foreach (var inPlayItem in boardState.Rectangles)
                    {
                        if (inPlayItem.localPlayer)
                        {
                            foreach(var deckItem in myDeck.CardsInDeck)
                            {
                                if (inPlayItem.cardCode == deckItem.Key)
                                {
                                    string newKey = deckItem.Key;
                                    int newCardCount = deckItem.Value - 1;
                                    myDeck.CardsInDeck.Remove(deckItem.Key);
                                    myDeck.CardsInDeck.Add(newKey, newCardCount);
                                    break;
                                }
                            }
                        }
        ...
        {
            string text = "";
            if (myDeck.CardsInDeck != null)
            {
                int singleImgHeight = 37;
                DeckList.Height = singleImgHeight * myDeck.CardsInDeck.Count;
                DeckList.Width = 300;
                ...
                using (Graphics graphics = Graphics.FromImage(outputImage)) {
                    int i = 0;
                    foreach (var item in myDeck.CardsInDeck)
                    {
                        ...
                        byte[] bytes = imageDownloader.DownloadData(allCards[item.Key].assets[0].gameAbsolutePath);
                        MemoryStream downloadStream = new MemoryStream(bytes);
                        Image assetImage = Image.FromStream(downloadStream);
                        ...
                        if (allCards[item.Key].type == "Unit")
                        {
                            ...
                            assetImage = ResizeImage(assetImage, (int)(assetImage.Width * scalingFactor), (int)(assetImage.Height * scalingFactor));
                            graphics.DrawImage(assetImage, new Rectangle(new Point(0, i*singleImgHeight), imgSize),
    ...
    public class Board
    {
        public List<card> Rectangles;
    }
    public class card
    {
        public string name { get; set; }
        public List<cardAsset> assets { get; set; } 
        public string cardCode { get; set; }
        public string supertype { get; set; }
        public string type { get; set; }
        public Boolean localPlayer { get; set; }
    }
